  ...
  namespace base_58 {
    namespace {
      ...
      struct decoded_block_sizes {
        decoded_block_sizes() {
          ...
          for (size_t i = 0; i <= full_block_size; ++i) {
            m_data[encoded_block_sizes[i]] = i;
          }
        }
      ...
      bool decode_block(const char* block, size_t size, char* res) {
        ...
        int decoded_size = decoded_block_sizes::instance(size);
        if (decoded_size <= 0)
          return false;
        ...
        if (res_num >> (decoded_size * 8))
          return false;

        uint_64_to_8be(res_num, decoded_size, reinterpret_cast<uint8_t*>(res));
