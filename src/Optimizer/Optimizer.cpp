...
namespace {
  ...
  const command_line::arg_descriptor<uint64_t>    arg_threshold = {"threshold", "Only outputs lesser than the threshold value will be included into optimization. Default: 100 (0.000100 xuni)", DEFAULT_THRESHOLD, true};
...
bool isWalletEligible(po::variables_map& vm, std::string address) {
  ...
  if (fusionReadyCount > 0) {
    logger(INFO, GREEN) << "Eligible Wallet: " << address << " Outputs: " << fusionReadyCount;
    return true;
  }
