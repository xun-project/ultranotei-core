...
void renameDataDir() {
  std::string ultranoteIDir = tools::getDefaultDataDirectory();
  boost::filesystem::path ultranoteIDirPath(ultranoteIDir);
  if (boost::filesystem::exists(ultranoteIDirPath)) {
    return;
  }

  std::string dataDirPrefix = ultranoteIDir.substr(0, ultranoteIDir.size() + 1 - sizeof(CRYPTONOTE_NAME));
  boost::filesystem::path cediDirPath(dataDirPrefix + "BXC");

  if (boost::filesystem::exists(cediDirPath)) {
    boost::filesystem::rename(cediDirPath, ultranoteIDirPath);
  } else {
    boost::filesystem::path BcediDirPath(dataDirPrefix + "Bcedi");
    if (boost::filesystem::exists(boost::filesystem::path(BcediDirPath))) {
		boost::filesystem::rename(BcediDirPath, ultranoteIDirPath);
    }
  }
}
...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    logger(INFO, BRIGHT_MAGENTA) << "- Daemon.cpp - " << cn::CRYPTONOTE_NAME << " v" << PROJECT_VERSION_LONG;
    ...
    cn::core ccore(currency, nullptr, logManager, vm["enable-blockchain-indexes"].as<bool>());
