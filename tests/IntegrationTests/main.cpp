...
#define CHECK_AND_ASSERT_MES_NON_FATAL(expr, fail_ret_val, message) \
  do                                                                \
  {                                                                 \
    if (!(expr))                                                    \
    {                                                               \
...
{
public:
  SimpleTest(const cn::Currency &currency, platform_system::Dispatcher &system, const Tests::common::BaseFunctionalTestsConfig &config) : BaseFunctionalTests(currency, system, config) {}
  ...
  bool perform1()
  {
    ...
    if (wallet2->pendingBalance() != 0)
      confirmed2.wait();
    if (wallet1->pendingBalance() != 0)
      confirmed1.wait();
    CHECK_AND_ASSERT_MES(stopMining(), false, "stopMining() failed");
    ...
    CHECK_AND_ASSERT_MES((w1ActualDiff - tr.amount - FEE == 0), false,
                         "STEP 7 FAILED\r\n wallet1 Actual Before Transaction doesn't match wallet1 Actual After Transaction + Transfered amount + Fee " + m_currency.formatAmount(wallet1ActualBeforeTransaction) + " <> " + m_currency.formatAmount(wallet1ActualAfterTransactionAndConfirmation) + "+" + m_currency.formatAmount(tr.amount) + "+" + m_currency.formatAmount(FEE));
  ...
  bool perform4()
  {
    ...
    if (!nodeDaemons[1]->submitBlock(test_block1_hex))
      return false;
    ...
    submitInvokingStart = std::chrono::steady_clock::now();
    if (!nodeDaemons[1]->submitBlock(test_block2_hex))
      return false;
    submitInvoked = std::chrono::steady_clock::now();
    ...
    submitInvokingStart = std::chrono::steady_clock::now();
    if (!nodeDaemons[1]->submitBlock(test_block3_hex))
      return false;
    submitInvoked = std::chrono::steady_clock::now();
    ...
    submitInvokingStart = std::chrono::steady_clock::now();
    if (!nodeDaemons[1]->submitBlock(test_block4_hex))
      return false;
    submitInvoked = std::chrono::steady_clock::now();
  ...
  bool perform5()
  {
    ...
    CHECK_AND_ASSERT_MES((w1PendingDiff - tr.amount - FEE == 0), false,
                         "STEP 5 ASSERTION 4 FAILED\r\n wallet1 Pending Before Transaction doesn't match wallet1 Pending After Transaction + Transfered amount + Fee " + m_currency.formatAmount(wallet1PendingBeforeTransaction) + " <> " + m_currency.formatAmount(wallet1PendingAfterTransaction) + "+" + m_currency.formatAmount(tr.amount) + "+" + m_currency.formatAmount(FEE));
    ...
    wallet2->addObserver(&confirmationObserver2);
    wallet1->addObserver(&confirmationObserver1);
    if (wallet2->pendingBalance() != wallet2->actualBalance())
      confirmed2.wait();
    if (wallet1->pendingBalance() != wallet1->actualBalance())
      confirmed1.wait();
    CHECK_AND_ASSERT_MES(stopMining(), false, "stopMining() failed");
    auto wallet1ActualAfterTransactionAndConfirmation = wallet1->actualBalance();
    auto wallet2ActualAfterTransactionAndConfirmation = wallet2->actualBalance();
